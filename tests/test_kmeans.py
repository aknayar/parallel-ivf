...
def test_kmeans_vs_sklearn_2d(indexes=None):
    ...
    data, true_centers = generate_gaussian_clusters(
        n_clusters=n_clusters, n_samples_per_cluster=100, dim=2
    )
    ...
    for ivf in indexes:
        ...
        assert (
            max_centroid_distance < EPSILON
        ), f"Centroids too different: max dist {max_centroid_distance:.2f}"
...
def test_kmeans_vs_sklearn_128d(indexes=None):
    ...
    n_clusters = 15
    data, true_centers = generate_gaussian_clusters(
        n_clusters=n_clusters, n_samples_per_cluster=1000, dim=128
    )
    n_samples, d = data.shape
    ...
    for ivf in indexes:
        ...
        assert (
            max_centroid_distance < EPSILON
        ), f"Max centroid distance {max_centroid_distance:.2f} too large"
